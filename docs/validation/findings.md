...
| F-019 | OPENSPIEL-ISMCTS | CRITICAL | *(found in a prior working session, registered retroactively — see Origin)* `engine_random_rollout` samples uniformly over unfiltered `engine_legal_moves()` output, so a UI-only `unavailable` placeholder move aborts 85-93% of rollouts onto a constant `{0,0}` leaf value instead of a real playout result |
| F-020 | ENGINE | MINOR | *(found by Review 01 of F-019, auditing the diff's own comments)* `engine_c/tests/test_rollout.c`'s file-header comment claims a post-fix unbounded rollout from a fresh 2p state reaches genuine terminal; the file's own test and live run show it hits `ROLLOUT_HARD_STEP_CAP` instead (`terminal=0`) |
| F-021 | VALIDATION-PROCESS | MAJOR | *(found by Review 01 of F-019's blast-radius re-run)* `docs/validation/harness/common.py::load()` defaults to `data/decks/base_setup.json`'s 10-card single-card market, which never offers a placeholder/modal move (0/1239 sampled states) — the entire harness battery (INV-1/2/3/6 and most F-*-blast-radius reruns) is provably blind to F-019's defect class and cannot validate behavior on the runner's real multi-deck market |
| F-022 | VALIDATION-PROCESS | MODERATE | *(found by Review 01 of F-019 while triaging non-baseline full-suite failures)* `tests/test_replay_player.py`/`test_replay_loader.py` hardcode exact values against the gitignored, locally-mutable `artifacts/ismcts/game_0000`; a locally-regenerated recording (1,308 steps vs. the hardcoded 517) fails all three tests with no git-diff-visible cause |
| F-023 | ENGINE | MINOR | *(found during a live performance-optimization session, 2026-09-04)* `engine_c/intern.c`'s global symbol table has zero synchronization — the one remaining big IS-MCTS speed lever (in-game leaf/rollout parallelism, since the C rollout call drops the GIL) is unsafe to pursue without fixing it first; evaluated by code inspection and **declined at owner direction**, not to be attempted |
...
- Review 04 [2026-09-03] at `63435fa` + uncommitted working-tree diff: **REJECTED-SCOPE**. An uncommitted, undocumented diff to `engine_c/scoring.c`/`scoring.h`/`view.c` reopens the narrower C-level accessor that Part B § 9 explicitly withdrew ("do not build it"), with no fix plan authorizing it and violating that plan's own G0 gate (zero diff in `engine_c/*.c`). No RED test precedes it (mandatory TDD constraint skipped). Status stays **CONFIRMED — FIXED** as Review 03 left it — nothing from this diff is merged.
- Diff: `engine_c/scoring.c`, `engine_c/scoring.h`, `engine_c/view.c` (uncommitted) | out-of-scope hunks: all of them (no authorizing plan exists) | test changes: 0 (none written for the new `_at` functions — itself a process violation, not a gaming case)
- Conformance: CONFORMANT — behaviorally a byte-identical, correctness-preserving refactor (independently verified: `state.c:101-105` index-aligns `state->nodes[]`/`board.nodes[]`; unmodified callers `compute_final_scores`/`award_end_of_turn_site_vp`/`helpers.c:233`/`player_view.c:49` still call the unmodified-behavior wrappers) — but correctness does not cure the scope violation
- Test: `harness/f011_timing.py` N=10 result=mean_wall=0.2755s, ratio vs pre-F-011 baseline (0.1646s) = **1.674×**, still under the 2× gate (run-to-run variance vs. Review 03's 0.2643s, not a demonstrated improvement)
- Regressions: none against F-013's own baseline — INV-1/2/3/6/4a/4b exact-match, INV-5 mean 11.48/12 min 8 exact-match, G1 byte-identity 12/12 (`test_board_projection.py`), F-010 `det_bot.py`/F-002 `f002b.py` unchanged, all 7 C suites PASS after `just build-c`, `openspiel_pyrants` suite only the 10 pre-existing postponed F-003 Part B RED failures, repo-root suite same 3 pre-existing failures **plus one new, unrelated failure** (`tests/test_replay_player.py::test_decision_at_matches_step`) in an untracked, out-of-domain WIP file sharing no code path with this diff — noted, not filed as a finding, not attributed to this diff
- Invalidated: STALE rows [none newly STALE — diff is rejected, not merged; INV-7/8/9 remain STALE from Review 01] | New findings raised: 0
...
### F-019 `engine_random_rollout` samples unfiltered UI placeholder moves, aborting most rollouts onto a constant zero leaf value
- Class: OPENSPIEL-ISMCTS
- Severity: CRITICAL (invalidates results — corrupts the majority of leaf values UCT backpropagates)
- Origin: **Found and fixed in a prior working session (2026-08-31, re-confirmed at 4 players 2026-09-04), outside this ledger's original static pass. The fix already existed in the working tree — with `docs/validation/findings.md`/`verdict.md` untouched and no fix-plan file — when this review session began. This entry is registered retroactively, at the owner's explicit direction, from that prior session's own measurements plus direct reading of the diff. Hard Rule 3 caveat, stated plainly: the falsification test below was written after the fix existed, not before it, and its "Observed" numbers are carried over from the prior session's own record rather than independently re-run in this session. This is a deviation from the pre-registration protocol, not a pre-registered test in the normal sense — any review of this finding must treat that as a standing caveat, not silently launder it into an ordinary CONFIRMED entry.**
- Code: `engine_c/rollout.c:9-33` (pre-fix, at `HEAD=2f6a0e7`) `engine_random_rollout` draws `int pick = (int)(rng_next(&rng) % (unsigned int)n);` directly over the raw array `engine_legal_moves(view, moves, ROLLOUT_MOVE_BUFFER)` fills, with no filtering step. `engine_c/generic_runtime.c:1274-1290` (`legal_pending_generic_choice_moves`) mixes a `MOVE_RESOLVE_GENERIC` move with `target_id = intern("unavailable")` into that same array for every non-viable modal option — a UI-only placeholder meant only so the Tkinter viewer can grey out the option, never meant to be applied. `engine_c/generic_runtime.c:734-742` (pre-fix `apply_resolve_generic_choice`) rejects exactly that move by returning `NULL`. `engine_c/rollout.c:29-30` (pre-fix) then does `GameState *next = engine_apply(view, &moves[pick]); if (!next) break;` — silently ending the rollout right there. The pre-fix non-terminal score branch (`rollout.c:37-42`) falls back to `view->players[i].score`, which omits site-control VP, trophy-hall VP, VP tokens, and deck-card VP (all summed only inside `scoring.c::compute_final_scores`), so it reads `0` for both players at most points in a game. `openspiel_pyrants/c_rollout_evaluator.py::CRolloutEvaluator` is the ISMCTS leaf evaluator that calls `adapter.random_rollout(seed, self.max_length)` and hands the result straight to `ISMCTSBot`'s backpropagation as a playout value.
- Rulebook: [absent] — pure engine/search-mechanics concern, not a rules-conformance question (same posture as F-010).
- Whitepaper: `docs/ismcts-paper.md:215` § III-A Monte Carlo Tree Search: "Simulated games select random actions until a terminal state is reached and the reward is averaged over multiple simulations to estimate the strength of each action."
- Disagreement: the paper's simulation step assumes a playout's reward reflects an actual outcome (terminal, or at minimum a state-dependent estimate). Here, at a representative 40-ply position on the runner's real setup (`_build_ismcts_setup_json`'s 74-card two-half-deck market — not `data/decks/base_setup.json`'s 10-card market, which was found to give misleadingly high terminal rates), the large majority of rollouts instead abort the instant RNG samples an `unavailable` placeholder, and report the constant fallback `0` regardless of the true board state. UCT backpropagation therefore averages a near-constant zero into node value estimates for most simulations, leaving only the rare real terminal rollout (~7-15% of the total) to carry any signal.
- Steelman: the placeholder-move mechanism is legitimate design on its own — it exists so the GUI can render a greyed-out option instead of hiding it, and other consumers (`compute_c_action_map` on the Python side, per the new `generic_runtime.c` comment) already filter it. The rollout's `if (!next) break;` is also a generic, reasonable defensive abort for *any* rejected move, not written specifically against this path. On this reading the defect is an unintended interaction between two independently sound designs (a GUI affordance, and a defensive null-check) rather than a rollout-specific oversight — though that does not change its effect on leaf values.
- Falsification test: from a representative 40-ply position on the runner's real setup (`_build_ismcts_setup_json`, both 2-player and 4-player), run N rollouts through `engine_random_rollout` (or its Python-replayed equivalent) at a range of `max_length` values including large/unbounded, and record: the fraction reaching a true terminal state, the specific rejection reason for every abort, and the reported score for aborted rollouts against the true board state.
- Expected if REAL: a large majority of rollouts abort specifically because `engine_apply` rejected an `unavailable`-tagged `MOVE_RESOLVE_GENERIC` move drawn from the unfiltered legal-moves list; raising `max_length` does not raise the terminal rate (ruling out the step cap as the cause); the reported score for aborted rollouts is `{0,0}` (or near-zero) independent of the true board state.
- Expected if FALSE POSITIVE: rollouts reach terminal (or abort only via the step cap) at a high rate regardless of `max_length`, and non-terminal scores vary with the true board state.
- Status: **CONFIRMED — FIXED** (working-tree change; not yet committed) — see Origin caveat above; not independently re-run by this ledger entry
- Command: prior-session ad hoc diagnostic replaying the C rollout loop (and, at 4p, tracing `engine_apply`'s rejection directly) against `_build_ismcts_setup_json`'s real setup — not yet promoted to a permanent `docs/validation/harness/*.py` script (same posture as F-015's ad hoc entry)
- N: 300 rollouts, 2p, 40-ply position (2026-08-31 session); 40 rollouts, 4p, 40-ply position (2026-09-04 session); post-fix 250 rollouts across ply {0, 10, 40, 120, 300} at both 2p and 4p
- Observed (carried over verbatim from the prior session's record; **not independently re-run in this session**):
  ```
  2p, 40-ply, N=300: 285/300 rollouts abort (95.0%); all 285 report {p1:0, p2:0} (mean 0, sd 0).
    15/300 reach terminal with real scores (e.g. {77,110}, {40,163}).
    max_length 200 -> 2000 -> 20000: terminal rate stays at 2/100 -- not a cap effect.
    Python-replayed C rollout loop: 45/45 aborts are engine_apply rejecting a
    target_id=="unavailable" MOVE_RESOLVE_GENERIC move drawn from the unfiltered list.
  4p, 40-ply, N=40: 34/40 rollouts abort (85.0%), 6/40 reach terminal.
  Post-fix, both player counts x 5 ply depths, N=250: 250/250 reach terminal, 0 aborts.
  ```
- Verdict rationale: matches "Expected if REAL" on all three measured axes (abort rate, abort cause, constant-zero score on abort), and the `max_length` sweep isolates the unfiltered placeholder move as the cause rather than the step cap. Post-fix figures show the filter restores a 100% terminal rate at both player counts across five ply depths. This rationale is carried over from the prior session's own analysis, not independently re-derived here — flagged per the Origin caveat.
- Resolution: `engine_c/generic_runtime.c` exports `move_is_unavailable_placeholder(const Move *move)` (declared in `generic_runtime.h`), factored out of the pre-existing reject-check inside `apply_resolve_generic_choice`. `engine_c/rollout.c::engine_random_rollout` now compacts `unavailable`-tagged moves out of `engine_legal_moves()`'s output in place before sampling, `break`ing only when every offered move is a placeholder, and always calls `compute_final_scores` for `scores_out` regardless of whether the rollout reached a true terminal state (a cut-off rollout now yields a real "score the game as if it ended here" estimate instead of the VP-incomplete raw `score` field). `ROLLOUT_HARD_STEP_CAP` is raised 2000 → 4000, with a measured terminal-rate-vs-cap table recorded in the new source comment, since the filtered rollout runs ~4.7× longer on average and the old cap — never hit pre-fix — sits only ~15% above the worst post-fix depth observed at 4p. `openspiel_pyrants/state_c.py::PyrantsCState.returns()` receives the matching Python-side fix: it now always calls `self._adapter.final_scores()` instead of branching on `is_terminal()` with the same raw-score fallback, so the C rollout evaluator and any direct `.returns()` caller report values on the same scale. `engine_c/bindings/ce_api.py::CEngine.random_rollout`'s docstring is updated to match. Tests: `engine_c/tests/test_rollout.c` (new `test_unavailable_placeholder_predicate` plus rollout-behavior assertions), `tests/c_engine/test_rollout_binding.py`, and new `openspiel_pyrants/tests/test_rollout_reaches_terminal.py`.
- Review 01 [2026-09-04] at `<working tree, not yet committed; base 2f6a0e7>`: **ACCEPTED-WITH-DEBT** [Hard Rule 3 registration debt (this finding was written retroactively — see Origin above); F-021 (harness battery blind to this defect class on the real market) and F-022 (unrelated, proven non-attributable) both open; 1 out-of-scope hunk] — `docs/validation/reviews/f019-review-01.md`
- Diff: `engine_c/{rollout.c,rollout.h,generic_runtime.c,generic_runtime.h,tests/test_rollout.c}`, `engine_c/bindings/ce_api.py`, `openspiel_pyrants/state_c.py`, `tests/c_engine/test_rollout_binding.py`, `openspiel_pyrants/tests/test_rollout_reaches_terminal.py` (new) | out-of-scope hunks: 1 (`.claude/CLAUDE.md`, Repowise auto-index refresh, assessed as inert/carried per the F-006 Review 01 precedent, not scope creep) | test changes: 4 hunks, all cleared against the Hard Rule 4 gaming checklist (2 renamed-and-strengthened assertions, 2 new files/tests, 0 weakened/skipped/widened/reduced-N/seed-pinned/special-cased/swallowed/disabled)
- Conformance: CONFORMANT vs rulebook [absent], paper § III-A ("Simulated games select random actions until a terminal state is reached and the reward is averaged... to estimate the strength of each action") — filtering out moves `engine_apply` refuses restores "random actions" to mean actions the game actually permits
- Test: `pytest openspiel_pyrants/tests/test_rollout_reaches_terminal.py` N=30 rollouts × {2p,4p} on the real two-half-deck market result=4/4 PASS, 30/30 terminal both player counts (was 15/300 at 2p, 6/40 at 4p pre-fix); `engine_c/tests/test_rollout.c` and `tests/c_engine/test_rollout_binding.py` both green with strengthened, non-tautological assertions
- Regressions: none attributable to this diff — INV-1/2/3/6 exact match (N=100/277/1200/30), F-010 `det_bot.py`/`det_isolate.py` unchanged, F-013 `f011_timing.py` unchanged (1.662×, still under 2× gate, though see F-021 caveat), F-002 `f002b.py` unchanged (8/8+8/8+8/8); 3 additional full-suite failures (`test_replay_player.py`×2, `test_replay_loader.py`×1) investigated and proven pre-existing/environment-local via a clean `git worktree` comparison at the same SHA, not caused by this diff — filed as F-022
- Invalidated: STALE rows [none newly STALE — INV-7/8/9 already STALE from prior reviews, this fix is a further independent trigger but was already correctly excluded from citation] | New findings raised: **F-020, F-021, F-022**

### F-020 `test_rollout.c`'s file-header comment claims post-fix unbounded rollouts reach genuine terminal; its own test shows they hit the step cap
- Class: ENGINE
- Severity: MINOR (misleading source comment; no behavioral effect)
- Origin: **Found by Review 01 of F-019, auditing the diff's own comments against its own tests, then confirmed by direct execution.**
- Code: `engine_c/tests/test_rollout.c:1-19` (file-header block comment, rewritten by the F-019 diff) states: "An earlier version of this note claimed a fresh game 'does not reach terminal within any practical random-rollout step budget'. That was a symptom... With those filtered out an unbounded rollout from a fresh 2p state runs to genuine terminal." The same file's `test_rollout_unbounded_completes_within_time_bound` (`:140-162`) exercises exactly that scenario (`test_load_definition`'s `base_setup.json` fixture, `max_length<=0`, fresh 2-player state) and carries its own, contradicting inline comment: "'Unbounded' (max_length<=0) still hits `ROLLOUT_HARD_STEP_CAP` in practice for a fresh game — this is a runaway-loop guard, not a promise of reaching genuine terminal." That test asserts only `ms < 5000.0`, never `terminal==1`.
- Rulebook: [absent]
- Whitepaper: [absent] — pure comment-accuracy issue.
- Disagreement: two comments in the same file, both touched by this diff, make opposite factual claims about the same fixture and code path; the live test run settles which one is true for this file's only fixture.
- Steelman: the file-header comment may be describing the real multi-deck market in the abstract (where the new Python test does show 30/30 terminal), and simply misplaced that claim under a file whose every fixture is `base_setup.json` — an imprecise generalization rather than a fabricated result.
- Falsification test: build and run `engine_c/tests/test_rollout.c` (`just build-c` / `cmd /c engine_c\compile.bat`), read the printed `terminal=` value on the "unbounded rollout completed" line.
- Expected if REAL: `terminal=0` (cap hit), contradicting the header's "runs to genuine terminal" claim.
- Expected if FALSE POSITIVE: `terminal=1`.
- Status: **CONFIRMED**
- Command: `cmd /c engine_c\compile.bat` (builds and runs `test_rollout.exe` as part of `just build-c`)
- N: 1 run (seed 99 is hardcoded in the test, so this is deterministic, not sampled)
- Observed:
  ```
  PASS: unbounded rollout completed in 25.0 ms (terminal=0)
  ```
  (captured live during this review, working tree = HEAD `2f6a0e7` + F-019's uncommitted diff)
- Verdict rationale: matches "Expected if REAL" exactly — `terminal=0`, confirming the file-header claim is false for the one fixture this file actually exercises. Purely cosmetic: does not affect rollout behavior, correctness, or any measurement, only a source comment's precision.

### F-021 The empirical validation harness's default market fixture never exercises the code path F-019 (and any modal-choice-gated defect) depends on
- Class: VALIDATION-PROCESS
- Severity: MAJOR (does not fabricate evidence, but the harness battery's PASS results do not demonstrate what they are cited to demonstrate for an entire class of behavior)
- Origin: **Found during Review 01 of F-019's BLAST RADIUS re-run**, after `f011_timing.py` showed a wall-time ratio essentially unchanged from the pre-fix baseline despite F-019's own fix comments measuring rollouts ~4.7× longer on the runner's real market — a discrepancy large enough to warrant direct verification rather than acceptance.
- Code: `docs/validation/harness/common.py:9-12` (`load()`) calls `pyspiel.load_game("python_pyrants_c", p)` with only `num_players` set, never `setup_data_json`. `openspiel_pyrants/game_c.py:18-24` (`_DEFAULT_SETUP`/`_DEFAULT_PARAMS`) resolves the unset param to `data/decks/base_setup.json` — a 10-copy single-card (`priestess_of_lolth`) market. Every `docs/validation/harness/*.py` script that calls `common.py::load()` without overriding `setup_data_json` — which is effectively the whole battery (`inv_a.py`, `det_bot.py`, `det_isolate.py`, `f002_reroll.py`, `f002b.py`, `f004_utility.py`, `f006*.py`, `f009.py`, `f010`/`f011`/`f013*_timing.py`, `findings.py`, etc.) — inherits this default.
- Rulebook: [absent]
- Whitepaper: [absent] — pure validation-methodology concern, same posture as F-016/F-017.
- Disagreement: `scripts/run_ismcts.py`'s actual runner builds a 74-card two-half-deck market via `_build_ismcts_setup_json`/`combine_two_deck_market_setup`, and F-019 is a defect its own fixing agent's test file documents as "invisible on `data/decks/base_setup.json`: its 10-card market ... ends games in about four rounds, before the modal cards whose options go non-viable are ever recruited" (`openspiel_pyrants/tests/test_rollout_reaches_terminal.py` module docstring). This review independently verified that claim: sampling `common.py::load(2)` under 20 seeds × up to 400 steps (1,239 sampled states, uniform random policy) found **zero** states offering a placeholder `resolve_generic` move. This means the whole `docs/validation/harness/` battery — including the INV-1/2/3/6 always-on invariants and this review's own F-010/F-013 blast-radius reruns — never exercises the pending-generic/modal-choice code paths F-019 changes, so a PASS there does not demonstrate anything about behavior on the runner's actual production configuration.
- Steelman: for invariants whose correctness doesn't depend on modal-choice frequency (INV-1 replay determinism, INV-2 clone independence, INV-6 chance mass, F-010's RNG-seeding mechanism, F-002's shuffle-reroll mechanism), the small market is a legitimate, cheap, faster-running proxy — those mechanisms are exercised regardless of market size. The gap specifically matters only for behavior gated behind pending-generic/modal resolution, and for any performance or strength claim that depends on realistic game length or card diversity — which is exactly F-019's territory.
- Falsification test: instrument `common.py::load()`'s underlying game with a placeholder-move counter (as this review did) across N sampled states under uniform random play; compare against an equivalent probe built on `_build_ismcts_setup_json`'s real market.
- Expected if REAL: the small-market probe shows 0 (or near-0) placeholder-move occurrences while the real-market probe shows many.
- Expected if FALSE POSITIVE: both probes show comparable placeholder-move rates.
- Status: **CONFIRMED**
- Command: ad hoc probe run in this review session (not yet promoted to a permanent `docs/validation/harness/*.py` script — same posture as F-015's ad hoc entry)
- N: 1,239 sampled states, 20 seeds (1-20), up to 400 steps each, uniform `RandomBot` policy, `common.py::load(2)` (default `base_setup.json` market)
- Observed:
  ```
  default (base_setup.json) market: states=1239 states_with_placeholder_move=0
  ```
- Verdict rationale: matches "Expected if REAL" exactly (0/1239), independently corroborating the fixing agent's own test-file docstring with a direct measurement rather than trusting the claim. This finding predates F-019 (the harness has always defaulted to `base_setup.json`) but is raised now because F-019 is the first ledger defect shown to be invisible under it, which is what exposes the gap's practical consequence.

### F-022 `test_replay_player.py`/`test_replay_loader.py` hardcode exact values against a gitignored, locally-mutable artifact, so a regenerated local recording fails them with no git-visible cause
- Class: VALIDATION-PROCESS (test design)
- Severity: MODERATE (currently-failing committed tests; not caused by any git-tracked change, but not caught by any diff review either)
- Origin: **Found while re-running the full repo `pytest -q` suite as part of F-019's RE-RUN step.** 3 failures appeared beyond the documented 3-item tolerated baseline (Zuggtmoy/Air Elemental/Neogi); isolated via a clean `git worktree add --detach` checkout at the same HEAD to rule out attribution to F-019's diff before filing this.
- Code: `tests/test_replay_player.py:44-63` (`test_full_replay_is_deterministic`, `test_total_steps_excludes_terminal_sentinel`) and `tests/test_replay_loader.py:158` (`test_load_replay_real_artifact_setup_sha_matches`) hardcode exact values (`player.index == 517`, `total_steps == 517`, `final_scores() == {"p1": 64, "p2": 93}`, `winner_id() == "p2"`) against the locally-present directory `artifacts/ismcts/game_0000`. That directory is declared in `.gitignore:235` and is not tracked by git at any commit.
- Rulebook: [absent]
- Whitepaper: [absent]
- Disagreement: `artifacts/ismcts/game_0000` on this machine currently holds a 1,308-step game (`steps.jsonl`/`replay.json` mtime 2026-09-04, generated locally by some prior run — not by this review, and not by anything in F-019's git-tracked diff, which touches neither `interface/replay_player.py`/`replay_loader.py` nor anything under `artifacts/`), so all three tests fail with `assert 1308 == 517`. A fresh `git worktree add` at the same commit has no `artifacts/` directory at all (gitignored), so the same tests instead SKIP there via the file's own `pytest.mark.skipif(not GAME_DIR.exists(), ...)` guard. The tests were evidently pinned once against a specific local recording and never re-guarded against that recording being *regenerated* — the module docstring anticipates `artifacts/` being *pruned* ("so the suite still passes after `artifacts/` is pruned") but not being present-with-different-content, which is the actual failure mode observed.
- Steelman: this is deliberate, cheap regression coverage for a real, expensive-to-reproduce end-to-end scenario (the "engine-seed-mixer trap" the docstring references), and pinning exact values against a golden recording is a legitimate technique that correctly skips when the recording is entirely absent — the gap is narrower than it first looks: only a present-but-different recording defeats it.
- Falsification test: `git worktree add` a clean checkout at the same commit (no local `artifacts/`), run `pytest tests/test_replay_player.py tests/test_replay_loader.py -v -rs`, and compare to the same command run in the main working tree.
- Expected if REAL: the clean worktree SKIPs the three tests (no artifact present) while the main working tree FAILs them with a value mismatch against a locally-regenerated artifact of different length/outcome.
- Expected if FALSE POSITIVE: both environments produce the same pass/fail/skip result, meaning the difference has some other, diff-attributable cause.
- Status: **CONFIRMED**
- Command: `git worktree add --detach ../pyrants-f019-baseline HEAD` then build + `pytest tests/test_replay_player.py tests/test_replay_loader.py -v -rs` in each of the worktree and the main working tree
- N: 3 tests, 1 baseline-vs-working-tree comparison, SHA `2f6a0e7` (both sides; working tree additionally carries F-019's uncommitted diff, confirmed irrelevant — neither side touches these files)
- Observed:
  ```
  clean worktree (git HEAD, no local artifacts/):  all 3 tests SKIP (GAME_DIR absent)
  main working tree (same HEAD + F-019 diff, local artifacts/ present, mtime 2026-09-04):
    test_full_replay_is_deterministic:                 assert 1308 == 517  FAIL
    test_total_steps_excludes_terminal_sentinel:       assert 1308 == 517  FAIL
    test_load_replay_real_artifact_setup_sha_matches:  assert 1308 == 517  FAIL
  ```
- Verdict rationale: matches "Expected if REAL". Confirms the 3 failures are pre-existing local-environment drift in an untracked artifact directory, not a defect in F-019's git-tracked diff — recorded here both as regression-attribution evidence for F-019 Review 01 (see that review's §4) and as an independent test-design gap in its own right.

### F-023 In-game rollout/leaf parallelism is blocked by `engine_c/intern.c`'s unsynchronized global symbol table — evaluated and declined

- Class: ENGINE
- Severity: MINOR (no current behavioral impact — nothing in this repo spawns concurrent engine calls today; this closes off a *future* optimization path, it does not describe a live defect)
- Origin: **Found during a live IS-MCTS performance-optimization session (2026-09-04)**, while chasing the one lever [[ismcts-performance-levers]] flags as unverified and `docs/validation/reviews/f019-review-01.md`'s handoff (§9) leaves unmeasured: "a rollout is one self-contained C call and ctypes drops the GIL, so within-game leaf parallelism could use the other 15 cores — unverified, needs the global intern table checked for thread-safety first." This finding is that check.
- Code: `engine_c/intern.c:16-21` declares six `static` globals (`table`, `table_size`, `entry_count`, `next_sym`, `sym_strs`, `sym_strs_cap`) with no mutex, atomic, or thread-local qualifier anywhere in the file (confirmed by reading it in full — 112 lines). `intern_len` (`:42-76`) is the only mutation path, and every card/site/move-field lookup in the engine funnels through it via `intern()` (`:78-81`). Three concrete races: (1) `:47-51` reads bucket head `table[idx]`, `:72-73` writes a new head back (`entry->next = table[idx]; table[idx] = entry;`) — two threads inserting into the same bucket concurrently can lose one insert; (2) `:53` `next_sym++` is a plain, non-atomic increment — two threads can hand out the *same* `Sym` for two different strings, which would make the engine treat two different cards/sites as identical downstream, silently; (3) `:54-66` grows `sym_strs` via `realloc`, which may move or free the backing allocation — a concurrent reader (`intern_str`, `:83-86`) or writer (`:62`, `:70`) touching the old pointer at that instant is a use-after-free.
- Rulebook: [absent] — pure engine-implementation concern, not a rules question.
- Whitepaper: [absent] — the paper's parallelization discussion (root/leaf parallelization as generic MCTS techniques) is orthogonal to whether *this engine's* shared state is safe to call concurrently; it does not address implementation-level thread-safety.
- Disagreement: not a code-vs-spec mismatch (this ledger's usual finding shape) — the gap is between an available-looking optimization (GIL-releasing C rollout calls, 15 idle cores while `--workers` today only parallelizes across games per `run_ismcts.py`) and the actual safety of taking it. The intern table is exactly the shape of unsynchronized global mutable state that turns "just add threads" into silent data corruption rather than a crash anyone would notice.
- Steelman: this is not a live bug. Nothing in this repo today calls into `engine_c` from more than one thread; `intern.c` was written for a single-threaded caller and has never needed locking under that contract. The absence of synchronization is a reasonable design for the code as it's actually used, not an oversight — it only becomes a hazard if within-game concurrency is added later without addressing it first.
- Falsification test (described, **not executed** — see Decision): spin up N threads (e.g. 8) all calling `intern()` concurrently against a small initial table, with enough new/overlapping strings across threads to force both bucket-collision inserts and at least one `sym_strs` `realloc` growth mid-race; then check (a) whether any two distinct strings were handed the same `Sym`, (b) whether every returned `Sym` round-trips through `intern_str()` back to its own string, and (c) whether the run survives under ThreadSanitizer (`-fsanitize=thread`) without a reported data race.
- Expected if REAL: at least one corrupted lookup (duplicate `Sym` for two strings, or a `Sym` that doesn't round-trip) or a TSan-reported race/UAF, reproducible across repeated runs.
- Expected if FALSE POSITIVE: the table stays internally consistent under concurrent load across many runs.
- Status: **CONFIRMED BY CODE INSPECTION ONLY — DECLINED, not to be pursued** (owner decision, 2026-09-04). No runtime falsification test was executed. Per this ledger's Hard Rule 3 posture (see F-019's own disclosed debt), a status resting on code reading alone is weaker than a pre-registered, executed test, and is recorded as such rather than dressed up as an empirical CONFIRMED — the code-level evidence (three independent unsynchronized-access sites, cited above with line numbers) is nonetheless unambiguous enough that this is not treated as a mere suspicion.
- Command: none run.
- N: n/a — no runtime measurement, source-reading only.
- Observed: n/a — see Code above for the citations standing in place of a runtime observation.
- Verdict rationale: The steelman (no live bug today) is accepted at face value; this finding exists to record that the specific optimization this ledger's own performance notes flagged as the biggest remaining IS-MCTS speed lever has a real, identified, unmitigated precondition, and that the precondition has been evaluated and rejected rather than left as a silent, unverified TODO.
- Decision [2026-09-04]: **DECLINED.** Owner directive, verbatim: "We're not doing that... not something we'd like to touch at all." In-game leaf/rollout parallelism is closed as a lever for this project — do not re-raise it. If ever revisited, two directions were identified but neither designed nor built: (a) a mutex around `intern_len`'s bucket-insert/counter/growth path (simplest, but serializes exactly the new-name lookups the parallelism targets, so its cost needs measuring before it's assumed to be a net win), or (b) a lock-free alternative — per-thread scratch tables reconciled after each rollout, or freezing/pre-warming the shared table before spawning threads so the parallel section only ever calls the read-only `intern_str` path. Neither is scoped, planned, or endorsed; this entry exists to prevent this lever from being silently re-suggested, not to authorize pursuing it later.
