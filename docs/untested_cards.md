...
- **Conjurer** — covered by `test_card_conjurer_behavior.py` (fuzzy naming match)
- **Vampie Spawn** — catalog typo for "Vampire Spawn", which IS tested (`test_card_vampire_spawn.py`)
- **Neogi** — test exists at `tests/test_card_neogi.py` but not in `tests/c_engine/`
- **cloaker**, **nalfeshnee** — lowercase names in catalog (data quality)
