"""Blackguard card behavior tests.

Verifies:
- Modal choice: gain 2 power OR assassinate
...
def _build_blackguard_session():
    eng = _make_engine()
    session = make_card_test_session(
        eng,
        ["p1", "p2"],
        hand={_P2: ["blackguard", "noble"]},
        spies={_SITE: [_P2]},
        current_player=_P2,
    )
...
def test_blackguard_gain_power():
    session = _build_blackguard_session()

    power_before = _resource_power(session)

    for m in session.legal_moves():
        if m.move_type == "play_card" and m.data.get("card_id") == "blackguard":
...
def test_blackguard_assassinate_white_troop():
    session = _build_blackguard_session()
    ...
    for am in assassinate_moves:
        if am.data.get("action_id") == _SITE and am.data.get("target_id") == "0":
            white_move = am
            break
    assert white_move is not None, f"White troop at {_SITE} should be targetable; moves: {[m.data for m in assassinate_moves]}"

    session.submit_move(white_move)
    assert not _has_pending_generic(session)

    trophy = _trophy_hall(session, _P2)
    assert _WHITE in trophy, f"Expected white in p2 trophy hall, got {trophy}"
...
def test_blackguard_assassinate_player_troop():
    eng = _make_engine()
    session = make_card_test_session(
        eng,
        ["p1", "p2"],
    ...
    has_p1 = any(t == "0" for t in target_ids if t is not None)
    assert has_p1, f"p1 troop (slot 0) should be targetable; targets: {target_ids}"
    ...
    session.submit_move(p1_move)

    trophy = _trophy_hall(session, _P2)
    assert "p1" in trophy, f"Expected p1 in p2 trophy hall, got {trophy}"

    session.destroy()


def test_blackguard_assassinate_requires_presence():
    eng = _make_engine()
    session = make_card_test_session(
        eng,
        ["p1", "p2"],
