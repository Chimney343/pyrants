"""Spectator card behavior tests: gain 2 power and 1 influence.

Spectator is a simple fixed-resource card with no timing or targeting
...
def test_spectator_gains_two_power_and_one_influence() -> None:
    """Playing Spectator grants exactly 2 power and 1 influence."""
    session = _build_session()

    power_before = _power(session)
    influence_before = _influence(session)

    _play_card(session, "spectator")

    assert _power(session) == power_before + 2, (
        f"Expected +2 power, got {_power(session) - power_before}"
    )
    assert _influence(session) == influence_before + 1, (
        f"Expected +1 influence, got {_influence(session) - influence_before}"
    )

    session.destroy()


def test_spectator_auto_resolves_without_pending_choice() -> None:
...
def test_spectator_moves_to_played_cards() -> None:
    ...
    assert "spectator" in played, f"Expected spectator in played_cards, got {played}"

    session.destroy()


def test_spectator_catalog_structure() -> None:
    ...
    for c in catalog["cards"]:
        if c["card_id"] == "spectator":
            card = c
            break
    assert card is not None, "Spectator not found in catalog"

    assert card["name"] == "Spectator"
    assert card["cost"] == 4
    assert card["aspect"] == "malice"
    assert card["secondary_aspects"] == ["aberration"]
    assert card["deck_vp"] == 2
    assert card["inner_circle_vp"] == 3
    assert card["rules_text"] == "Gain 2 power and 1 influence."
    ...
    assert a2["quantity"] == {"kind": "fixed", "value": 1}
