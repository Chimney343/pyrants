"""Howling Hatred Cultist card behavior tests.

Modal choice: choose exactly one mode.
- Option 1: Place a spy at a board site.
- Option 2: Return one of your spies and gain 3 influence. Guile focus: gain 1 power.
"""
...
def _build_session(**kwargs):
    defaults = {
        "player_ids": [_P1, _P2],
        "hand": {_P1: ["howling_hatred_cultist"]},
        "current_player": _P1,
    }
...
def _play_card(session, card_id="howling_hatred_cultist"):
...
def test_option_2_return_spy_with_focus_bonus():
    """Choose option 2 with guile focus card in hand: return spy, gain 3 influence + 1 power."""
    session = _build_session(
        hand={_P1: ["howling_hatred_cultist", "infiltrator"]},
        spies={_SITE: [_P1]},
    )
    ...
    assert _resource_power(session) == power_before + 1, "Should gain 1 power from guile focus"

    session.destroy()


def test_option_2_return_spy_without_focus_gains_only_influence():
    """Choose option 2 without guile focus: return spy, gain 3 influence, no power."""
    session = _build_session(
        spies={_SITE: [_P1]},
    )
    spies_avail_before = _spies_available(session, _P1)
    ...
    assert _spies_available(session, _P1) == spies_avail_before + 1, "Returned spy should be available"
    assert _resource_influence(session) == influence_before + 3, "Should gain 3 influence"
    assert _resource_power(session) == power_before, "Should NOT gain power without guile focus"
